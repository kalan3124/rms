...
class SrChemistController extends Controller{
     ...
     public function srChemistData(Request $request){
          ...
          if($json_decode['update_status'] == true){
               ...
               // return $json_decode;die;

               $sr_chemsit_update = DistributorCustomer::where('dc_code',$json_decode['chem_code'])->first();
               // $sr_chemsit_update->mobile_number = $json_decode['mobile'];
               // $sr_chemsit_update->email = $json_decode['email'];
               $sr_chemsit_update->dc_address = $json_decode['address'];
               $sr_chemsit_update->dc_lat = $json_decode['lat'];
               $sr_chemsit_update->dc_lon = $json_decode['lon'];
               $sr_chemsit_update->dc_image_url = $imgUrl;
               // $sr_chemsit_update->updated_u_id = $user->getKey();
               // $sr_chemsit_update->phone_no = $json_decode['telephone'];
               $sr_chemsit_update->dc_name = $json_decode['outlet_name'];
               ...
               $sr_chemsit = SrChemist::create([
                    'dc_name' => $json_decode['outlet_name'],
                    // 'owner_name' => $json_decode['owner_name'],
                    'dc_address' => $json_decode['address'],
                    // 'mobile number' => $json_decode['mobile'],
                    // 'email' => $json_decode['email'],
                    'dc_lat' => $json_decode['lat'],
                    'dc_lon' => $json_decode['lon'],
                    'dc_image_url' => $imgUrl,
