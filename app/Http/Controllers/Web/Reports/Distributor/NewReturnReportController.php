...
class NewReturnReportController extends ReportController
{

    protected $title = "New Return Report";
    ...
    public function search(Request $request)
    {
        ...
        $query = DB::table('distributor_return_item AS rl')
            ...
            ->select([
                ...
                'dc.dc_name',
                'dc.dc_code',
                DB::raw('SUM((dri_bns_qty + dri_qty) * dri_price) amount')
            ])
            ->groupBy('dist_return_number');
        ...
        foreach ($results as $key => $userss) {
            ...
            $rowNew['rtn_value'] = number_format($userss->amount, 2);
...

    {
        ...
        $inputController->setStructure([['customer', 'distributor'], ['salesref','invoice_code'], [ 's_date', 'e_date']]);
    }
}
