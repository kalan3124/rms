...
class YtdExpnesesReportController extends ReportController {

     use Team,Territory;
     protected $updateColumnsOnSearch = true;
     protected $title = "Ytd Expenses Report";
     ...
     public function search(Request $request){
          ...
          $count = 0/*$this->paginateAndCount($query,$request,'t.tm_id')*/;
          $results = $query->get();
          ...
          $results->transform(function($data) use($period,$values,$end){
               ...
               $YeaMonthsAchi = $this->makeQuery($towns,date('Y-01-01',strtotime($values['s_date'])),date('Y-m-t'),$teamUser?$teamUser->tm_id:0,$teamUser?$teamUser->u_id:0);
               ...
               foreach ($period as $key => $month) {
                    ...
                    $currentMonthAchievement = $this->makeQuery($towns,date('Y-m-d',strtotime($month->format('Y-m-d'))),date('Y-m-t',strtotime($month->format('Y-m-d').'+1 month')),$teamUser?$teamUser->tm_id:0,$teamUser?$teamUser->u_id:0);
                    // $currentMonthAchievement = $this->makeQuery($towns,date('Y-m-01',strtotime($month->format('Y-m-d'))),date('Y-m-t',strtotime($month->format('Y-m-d'))));
     ...
     protected function setColumns(ColumnController $columnController,Request $request){
          ...
          if(isset($values['s_date'])){
               ...
               foreach ($period as $key => $month) {
                    $columnController->number('ori_target_val'.$month->format('m'))->setLabel($month->format('M-d').' '."Original Target Value");
                    $columnController->number('month_ach_val'.$month->format('m'))->setLabel($month->format('M-d').' '."Month Ach Value");
                    $columnController->number('ach%'.$month->format('m'))->setLabel($month->format('M-d').' '."Ach%");
               }
          }
     ...
     protected function setInputs($inputController){
          ...
          $inputController->ajax_dropdown("user_id")->setLabel("PS/MR")->setWhere(['u_tp_id'=>'3'.'|'.config('shl.product_specialist_type')])->setLink("user")->setValidations('');
          $inputController->date("s_date")->setLabel("Financial Month");
          $inputController->text("year")->setLabel("Financial Year");

          $inputController->setStructure([
               ["team_id","div_id","user_id"],["principle_id","s_date"]
               ]);
     }
}
