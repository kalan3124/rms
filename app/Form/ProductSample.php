...
class ProductSample extends Form{

    use Team;

    protected $title = 'Sample Product';

    protected $dropdownDesplayPattern = 's_product_name';
    ...
    public function beforeSearch($query,$values){
        ...
        if(in_array($user->getRoll(),[
            ...
            $sample_products = $this->getSampleProductsByUser($user);

            $query->whereIn('product_id',$sample_products->pluck('product_id')->all());
        }
    }
    ...
    public function beforeDropdownSearch($query, $keyword)
    {
        ...
        if($teams->count()){
            ...
            $query->whereIn('product_id',$sample_products->$users->pluck('product_id')->all());
        }
    }
    ...
    public function setInputs(\App\Form\Inputs\InputController $inputController)
    {
        ...
        $inputController->text('s_product_code')->setLabel('Sample Product Code')->isUpperCase();
        $inputController->text('s_product_name')->setLabel('Sample Product Name')->isUpperCase();
        // $inputController->ajax_dropdown('sample_product_family_id')->setLabel('Product Family')->setLink('product_family');
        ...
        $inputController->setStructure([
            'product_id',
            ['s_product_code','s_product_name'],
            // ['principal_id','sample_product_family_id'],
            // ['brand_id','divi_id']
        ]);
    }
