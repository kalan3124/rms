...
class TargetVsAchievementReportControllerNew extends ReportController {
    use Team,Territory;
    ...
    public function search(Request $request){
        ...
        $lastYearThisMonth =  DB::table('month_wise_achievement')
        ...
        ->where('mwa_month',date('m',strtotime($month.' - 1 year')))
        ->where('mwa_year',date('Y',strtotime($month.' - 1 year')))
        ->get();

        $lastYearThisMonth = collect($lastYearThisMonth)->map(function($x){ return (array) $x; });

        $ytd =  DB::table('month_wise_achievement')
        ->where('u_id',$user->getKey())
        ->groupBy('product_id')
        ->select([
             'product_id',
        ...
        ->where('mwa_month','<',date('m',strtotime($month)))
        ->where('mwa_year',date('Y',strtotime($month)))
        ->get();
        $ytd = collect($ytd)->map(function($x){ return (array) $x; });

        $currentMonthAchievement = $this->makeQuery($towns,date('Y-m-01',strtotime($month)),date('Y-m-t',strtotime($month)),$teamUser?$teamUser->tm_id:0,$user?$user->id:0);
        ...
        foreach($products as $product){
            ...
            $currentMonthQty = $currentMonthAchievementProduct?$currentMonthAchievementProduct['qty']:0;
            $currentMonthAmount = $currentMonthAchievementProduct?$currentMonthAchievementProduct['amount']:0;
            $ytdQty = $ytdProduct?$ytdProduct['qty']:0;
            $ytdAmount =  $ytdProduct?$ytdProduct['amount']:0;

            $ytdQty+= $currentMonthQty;
            $ytdAmount+= $currentMonthAmount;
            ...
            $tot_cur_ach_qty += $currentMonthQty;
            $tot_cur_targ_val += $userProductTarget?$userProductTarget->upt_value:0;
            $tot_cur_ach_val += $currentMonthAmount;
            ...
            $tot_ytd_ach_qty += $ytdQty;
            $tot_ytd_cur_ach_val += $ytdAmount;
            ...
            $results[]  = [
                ...
                'cur_ach_qty'=>$currentMonthQty?$currentMonthQty:0,
                'cur_targ_val'=>number_format($userProductTarget?$userProductTarget->upt_value:0,2),
                'cur_ach_val'=>number_format($currentMonthAmount,2),
                'cur_ach_%' => $achi_prasant?round($achi_prasant,2):'0.00',
                ...
                'ytd_ach_qty'=>$ytdQty?$ytdQty:'0',
                'ytd_cur_ach_val'=>number_format($ytdAmount,2),
    ...
    public function setColumns($columnController, Request $request){
        ...
        $columnController->number('price')->setLabel('Price')->isSearchable(false);
        $columnController->number('cur_targ_qty')->setLabel('Target Qty')->isSearchable(false);
        $columnController->number('cur_ach_qty')->setLabel('Achiev. Qty')->isSearchable(false);
        $columnController->number('cur_targ_val')->setLabel('Target Value Rs.')->isSearchable(false);
        $columnController->number('cur_ach_val')->setLabel('Achiev. Value Rs.')->isSearchable(false);
        $columnController->number('cur_ach_%')->setLabel('Achiev %.')->isSearchable(false);
        $columnController->number('lst_yr_cur_mnth_ach_qty')->setLabel('Achiev. Qty')->isSearchable(false);
        $columnController->number('lst_yr_cur_mnth_cur_ach_val')->setLabel('Achiev. Value Rs.')->isSearchable(false);
        $columnController->number('ytd_ach_qty')->setLabel('Achiev. Qty')->isSearchable(false);
        $columnController->number('ytd_cur_ach_val')->setLabel('Achiev. Value Rs.')->isSearchable(false);
        $columnController->number('growth')->setLabel('Growth %')->isSearchable(false);
    }
