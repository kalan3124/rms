...
class UserAttendanceController extends Controller{
    ...
    public function attendance(Request $request){
        ...
        if ($json_decode['type'] == 1) {
            ...
            if($lastAttendance&&!$lastAttendance->isDayEnded()) throw new MediAPIException("Day begin has been already exist!",6);        
            ...
            try{
                ...
                throw new MediAPIException("Day begin has been already exist!",6);
            }
        } else {

            // Throw an error if can not find one
            if(!$lastAttendance) throw new MediAPIException("Can not find a day begin related to user",7);

            // Throw an error if already day ended
            // if($lastAttendance->isDayEnded()) throw new MediAPIException("Already day ended!",8);
    ...
    public function dayStatus (Request $request) {
        ...
        if(!$attendance)
            throw new MediAPIException("Can not find a attendance related to user",9);
