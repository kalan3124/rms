...
import {changeValue, fetchData } from "../../../../actions/Medical/MrPsItineraryReport";
import { MEDICAL_FIELD_MANAGER_TYPE, MEDICAL_REP_TYPE, PRODUCT_SPECIALIST_TYPE } from '../../../../constants/config';
...
class MrPsItineraryReport extends Component{
          ...
          return (
              <Layout sidebar>
                  <Paper className={classes.padding} >
                      <Typography variant="h5" align="center">MR PS Itinerary Report</Typography>
                      <Divider />
                      <Grid container>
                        ...
                        <Grid className={classes.padding} item md={5}>
                            <AjaxDropdown where={{ tm_id: '{tm_id}',divi_id:'{divi_id}',u_tp_id:MEDICAL_REP_TYPE+'|'+PRODUCT_SPECIALIST_TYPE }} otherValues={{ tm_id: values.team, divi_id:values.division }} value={values.user} label="MR/PS" onChange={this.changeFormValue('user')}  link="user" name="u_id" />
                        </Grid>
                        <Grid className={classes.padding} item md={5}>
                            <AjaxDropdown value={values.division} label="Division" onChange={this.changeFormValue('division')}  name="division" link="division" />
                        </Grid>
                        <Grid className={classes.padding} item md={5}>
                            <DatePicker value={values.e_date} label="From" onChange={this.changeFormValue('e_date')}  name="e_date" />
                        </Grid>
                        <Grid item md={10}>
                            <Divider />
     ...
     renderTable() {
      ...
      renderDayPlanTable(){
          ...
          rowData.forEach(element => {
            if(parseInt(element[3]) != 0 & element[3] != null){
                km +=parseInt(element[3]);
            }

              if(element[1] == "WORKING DAY"){
                  wk++;
              }
              if(element[1] == "SPM"){
                  spm++;
...
MrPsItineraryReport.propTypes = {
 ...
 export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(MrPsItineraryReport));