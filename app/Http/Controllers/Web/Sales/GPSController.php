...
class GPSController extends Controller{
    public function search(Request $request){
        ...
        // })->where('u_id',$userId)->limit(1)->latest()->get();

        $checkings = UserAttendance::where('u_id',$userId)->whereDate('check_in_time',$fromDateTime)->get();
        $checkinTime = null;
        $checkoutTime = null;

        if(!isset($checkings)){
            throw new WebAPIException('No Any Attendance for Date');
        }
